  ...
  return (
    <header className="sticky top-0 z-20 border-b border-[#ded8c9] bg-[#faf8f3]/95 px-4 py-3 backdrop-blur md:px-7">
      <div className="mx-auto flex max-w-[1400px] flex-wrap items-center gap-3">
        ...
        <div className="flex max-w-full items-center gap-2 rounded-full border border-[#ded8c9] bg-white px-2.5 py-1.5 text-xs text-neutral-700">
          <span
            className={cn(
      ...
      <div className="divide-y divide-[#eee8dc]">
        ...
        <TraceSection title="Graph hops">
          <div className="space-y-1.5">
            {response.graphHops.map((hop) => (
              <div
                key={`${hop.source}-${hop.relationship}-${hop.target}`}
        ...
        <TraceSection title="Knowledge chunks">
          <div className="space-y-2">
            {response.chunks.map((chunk) => (
              <div key={chunk.title} className="border-t border-dashed border-[#eee8dc] pt-2 first:border-t-0 first:pt-0">
        ...
        <TraceSection title="Memory writes">
          <div className="space-y-1.5">
            {response.profileWrites.map((write) => (
              <div key={`${write.label}-${write.value}`} className="flex gap-2 font-mono text-[11px]">
      ...
      {showProducts ? (
        <div className="space-y-3">
          {response.picks.map((pick, index) => {
            const product = products[pick.productId];
            if (!product) {
              return null;
            }
            ...
            if (!visible) {
              return (
                <Card key={pick.productId} className="rounded-lg border-[#ded8c9] bg-white shadow-sm">
      ...
      {showPaired ? (
        <div className="space-y-2 pt-2">
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {response.pairedProductIds.map((productId) => {
              const product = products[productId];
              return product ? <SmallProductCard key={productId} product={product} /> : null;
            })}
          </div>
        </div>
      ) : null}
        ...
        {showPath ? (
          <div className="grid items-stretch gap-2 lg:grid-cols-[1fr_auto_1fr_auto_1fr]">
            {response.path.map((step, index) => (
              <PathStep key={`${step.kind}-${step.label}`} step={step} isLast={index === response.path.length - 1} />
            ))}
          </div>
        ) : (
          <Skeleton className="h-24 w-full" />
          ...
          <div className="space-y-2">
            ...
            <ol className="space-y-2">
              {response.actions.map((action, index) => (
                <li key={action} className="grid grid-cols-[2rem_1fr] gap-2 border-t border-[#eee8dc] pt-2 text-sm text-neutral-700 first:border-t-0 first:pt-0">
        ...
        {showAlternatives && response.alternativeProductIds.length ? (
          <div className="space-y-2">
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              {response.alternativeProductIds.map((productId) => {
                const product = products[productId];
                return product ? <SmallProductCard key={productId} product={product} /> : null;
              })}
            </div>
...
function DemoClient() {
  ...
  async function submit(presetId?: string, prompt?: string) {
  ...
  return (
    <div className="min-h-screen bg-[#f4f2ec] text-neutral-950">
      <Header mode={mode} setMode={setMode} profileChips={profileChips} reset={reset} />
